...
// demas variables en fun_web.js o fun_nw.js
 	...
 	// Actualizacion Capa Skill ESC (Cierra la capa segun sea la que esta abierta con la tecla ESC, BETA 1)
var CajaSkill = 0; // caja simple
var CajaSkill2 = 0; // teatro
...
// Gral
	...
	// lanza dimensiones sin distorcionar, Ancho o Max alto segun sea el tamaño real de la imagen
	function addDim(AnO,AlO,ancho,hmax){
		if(ancho == 0 && hmax > 0 && hmax <= AlO) {
			...
			var ratio = (AnO / ancho);
			alto = (AlO / ratio);
			if(alto>hmax){ var anchura2 = hmax*ancho/alto; alto = hmax; ancho = anchura2; }

			return ancho+'x'+alto;
		}
		else if(ancho > 0 && ancho <=AnO && hmax == 0) {
			var width = AnO;
    		var new_width = ancho; //nuevo tamaño
    		if (width > new_width){
      			var height = AlO;
...
function jQ_valEmail(id){
	...
	function jQ_OnlyNumber(id,atributo){
		var atributo = (atributo) ? atributo : 'onmouseover';
		$('#'+id).removeAttr(atributo); // quitamos propiedad aplicadora de funcion del objeto
		$('#'+id).keydown(function(e) {
           ...
           if ((e.shiftKey || (e.keyCode < 48 || e.keyCode > 57)) && (e.keyCode < 96 || e.keyCode > 105)) {
			...
			var key = window.Event ? evt.which : evt.keyCode; 
			// seteamos datos
			if($('#'+id).attr('data-modo')){ var modo = $('#'+id).attr('data-modo'); } else { var modo = 'entero'; }
			if($('#'+id).attr('data-total')){ var total = $('#'+id).attr('data-total'); } else { var total = 10000000000; }
			if($('#'+id).attr('data-len')){ var mlen = $('#'+id).attr('data-len'); } else { var mlen = 25; }
			total = parseInt(total); mlen = parseInt(mlen);

           if(this.value.length > mlen) { this.value = this.value.substring(0,mlen); }
           if(this.value > total) { this.value = this.value.substring(0,mlen-1); }
           if(this.value == 0) { this.value = ''; } //  || key == 32
	   });	 
	}


	function valUploadImagen(id){
		var _URL = window.URL || window.webkitURL;
		$('#'+id).removeAttr('OnMouseOver'); // quitamos propiedad aplicadora de funcion del objeto
		
		// activamos funcion JQ del objeto
	    	...
	    	var image, file;
      		...
      		if ((file = this.files[0])) {
		    	...
		    	var sizeByte = this.files[0].size;
	       		...
	       		var nomFile = this.files[0].name;
	       		var fiExt = nomFile.substr(-3); fiExt = fiExt.toLowerCase();

    			// Valores para verificacion
		        var dim = $('#'+id).attr('data-dim'); dim = dim.split('x'); // dimensiones max
		        var imgKB = $('#'+id).attr('size');  // peso max
		        var ext = $('#'+id).attr('data-ext'); ext = ext.split(','); // ext permitidas
		        var preview = $('#'+id).attr('data-preview'); // aceptamos preview
		        var preAn = $('#'+id).attr('data-an'); var preHm = $('#'+id).attr('data-hm'); // dimensiones de preview

		        // creamos imagen temporal
    		    image = new Image();
		        image.onload = function() {   

		        	if(this.width <= dim[0] && this.height <= dim[1]) { // Dimensiones permitida?
		        		if(sizeKB <= imgKB) { // peso permitido ?
		        			if(ext[0] == fiExt || ext[1] == fiExt || ext[2] == fiExt) { // Extension permitida?
		        				//Permito Preview?
		        				if(preview == 'true') { 
		        					var DimMini = addDim(this.width,this.height,preAn,preHm); DimMini = DimMini.split('x');
         							var imgN = document.getElementById(id+'_Preview');
         							imgN.src = _URL.createObjectURL(file);
         							imgN.width = DimMini[0];
         							imgN.height = DimMini[1];
         						}
         						// aviso que no hay error al verificar imagen lado Cliente
		        				AvisoTop('Exito S3','Imagen Inspeccionada con éxito...',null);
                 			}
                 			else {
								AvisoTop('Aviso S3','Solo se aceptan JPG, PNG, GIF ==> ( Imagen Actual: ' +fiExt+ ')',null);
          						$('#'+id).val('');
                 			}
       	 				}
        				else{
          					AvisoTop('Aviso S3','El peso de la imagen permitido es de: '+imgKB+'Kb ==> (Imagen Actual: ' + sizeKB + ' Kb)',null);
          					$('#'+id).val('');
          				}
        			}
        			else {
						AvisoTop('Aviso S3','Las dimensiones permitidas son menor o igual a ' + dim[0]+'x'+dim[1] + 'pixeles.  ==> (Imagen Actual: '+this.width+'x'+this.height+'Px)',null);
						$('#'+id).val('');
        			}
...
//	function Max(id,SpCon,Max){ var Limit = Max; if(id.value.length <= Max) {TipBox(SpCon,id.value.length);}else {id.value = id.value.substring(0,Limit);}}
	...
	//Envio  de formularios version para inputs
...
// cajas
	...
	//function Caja(){ CajaSkill = 1; $('body#Cuerpo').css({'overflow':'hidden'}); $('div#Caja_Viewer,div#Caja_Shadow').fadeIn(); }
...
//	function CajaOff(){ CajaSkill = 0; $('body#Cuerpo').css({'overflow':'auto'}); $('div#Caja_Viewer,div#Caja_Shadow').fadeOut(); var ControlCaja = setTimeout(function(){TipBox('div#Caja_CConten','')},5000);}
	function TeatroOn(){ CajaSkill2 = 1; $('body#Cuerpo').css({'overflow':'hidden'}); $('div#Teatro_Shadow,div#Teatro_Viewer').fadeIn(); }
	function TeatroOff(){ CajaSkill2 = 0; $('body#Cuerpo').css({'overflow':'auto'}); $('div#Teatro_Viewer,div#Teatro_Shadow').fadeOut(); var ControlCaja2 = setTimeout(function(){TipBox('div#Teatro_CConten','')},5000);}
	...
	$(document).keyup(function(e) { if(e.keyCode == 27){ if(CajaEsc[0] == 1) { CajaOff(); } if(CajaSkill2 == 1) {TeatroOff();} } });

// Menu Tabulador Horizontal
var TabHLoad = Array(10,10);
for(i = 0; i < TabHLoad.length ; i++) {TabHLoad[i]=0;}
function Tab(tag,id,max){tagT = '#'+tag+'-Tab_';tagC = 'div#'+tag+'-Capa_';TabHLoad[tagT+id]=id;for(var i = 0; i <= max; i++) {if(i==id){$(tagT+id).addClass('Activo');$(tagC+id).fadeIn();}else {if(TabHLoad[tagT+id]!=i) {$(tagT+i).removeClass('Activo'); $(tagC+i).fadeOut();}}}}
...
*     Autores: Ismael Alfredo Martinz Escobedo 
